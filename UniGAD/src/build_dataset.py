...
def build_unigad_dataset(original_file_path, output_dir):
    """
    加载一个DGL图文件，确保其键名为'node_label'和'edge_label'，
    然后保存为与原始UniGAD代码完全兼容的格式。
    """
    print(f"--- 正在处理文件: {original_file_path} ---")
    ...
    if os.path.exists(output_file_path):
        print(f"文件 {output_file_path} 已存在，跳过。")
        return
    ...
    try:
        ...
        if 'label' in g.ndata:
            ...
            print("错误：在节点数据中未找到任何可用的标签键 ('label' 或 'node_label')。")
            return
...
if __name__ == "__main__":
    ...
    target_folder = '../datasets/edge_labels' # UniGAD代码期望的目录

    if not os.path.isdir(source_folder):
        print(f"错误: 提供的路径 '{source_folder}' 不是一个有效的文件夹。")
        sys.exit(1)

    print(f"开始批量处理文件夹 '{source_folder}' 中的所有数据集...")
